...
- **LLM + Stream ON**：使用真实 SSE 流式输出，详见"SSE 流式模式"章节
...
5. App 自动用 `A2uiJsonValidator` 校验 LLM 输出，验证失败时 Debug 面板显示原始输出和错误详情，不会发送到 SDK 渲染
...
5. App 收到连接后调用 `surfaceManager.beginTextStream()`，每个 delta 调用 `receiveTextChunk(delta)`，收到 `[DONE]` 后调用 `endTextStream()`
6. 流结束后，对累积的完整文本做 `A2uiJsonValidator.validateFromRawText()` 后验证
...
取消支持：再次点击生成按钮、切换 Provider 或退出 Activity 时，关闭 SSE 连接并调用 `endTextStream()`，不弹出错误 Toast。
...
发送给 SurfaceManager 前，A2UI JSON 会自动校验：
...
- 实现真实 LLM SSE 流式输出直连 `receiveTextChunk`
