...
| Card 退化为透明 / 无形 | 启用 mluiCard variant 但卡片不见了 | `melo-card-background` 资源解析失败 + variant fallback 装饰被设成透明 / 0px | theme JSON 中 Card variant 的 fallback 必须保持可见（`#FFFFFF + 16px/0px + filter:none`），不能用透明压制；公司控件可用时仍由 `MeloCardComponent.applyCardAppearanceIfPresent()` 覆盖为公司色 |
| Card 日夜不切换 | dark mode 下 mluiCard 仍是 day 色 | `applyCardAppearanceIfPresent()` 走了 `getColor()` 路径，把 ColorStateList 拍平成单色 | 必须先 `ContextCompat.getColorStateList()` + `GradientDrawable.setColor(ColorStateList)`；只有 ColorStateList 拿不到才退到 `getColor()` |

### 公司共通控件 SDK 适配

Demo 通过 `CommonControlsRegistry` 在 A2UI 渲染层适配公司 Melo/Gua 控件。完整里程碑见 `docs/demo-milestone-common-controls-sdk-v0.1.md`。

- **AAR 位置**：`demo/android-a2ui-text-demo/sdk/gua-car-ui-lib-release.aar`（不提交 Git）
- **注册入口**：`CommonControlsRegistry.registerIfAvailable(aGenUI, context)`，在 `AGenUI.initialize()` 之后、`SurfaceManager` 创建之前调用
- **反射加载**：所有 Melo 控件通过 `Class.forName()` 反射创建，无编译期依赖。反射失败自动 fallback 到标准 Android View
- **A2UI 协议无感知**：A2UI JSON 仍使用标准组件名，CardData 协议不变
- **已适配组件**：Text → MeloTextView、Card → MeloFrameLayout（fallback FrameLayout）+ GradientDrawable、Button → MeloFrameLayout 容器。Row/Column 保留原生 FlexContainerLayout（LinearLayout 替代会丢失 Flexbox 语义）
- **minSdk 28**：因公司 AAR 要求
- **验证方法**：App 日志 `"Common controls available: true/false"`

#### Card backing view 是 FrameLayout，不是 CardView

历史上 A2UI Card 用 MeloCardView/AndroidX CardView 承载，结果踩了一长串坑：`setPadding` 设到外层而不是内容区（要特判 `setContentPadding`）、自带 `cardBackgroundColor / radius / cardElevation` 抢视觉层、`useCompatPadding / preventCornerOverlap` 与 Flexbox 父容器打架。**当前 Card backing view 已改为 MeloFrameLayout / FrameLayout + `GradientDrawable` 背景 + 普通 `setPadding`**：

- `CommonControlsViewFactory.createCardView()` 只反射 `MeloFrameLayout`，fallback 到 `android.widget.FrameLayout`
- `MeloCardComponent` 删除了全部 CardView 专属 API（`setContentPadding / setCardBackgroundColor / setRadius / setCardElevation / setUseCompatPadding / setPreventCornerOverlap / instanceof CardView`），padding 走普通 `setPadding`
- `melo-card-background` 资源解析顺序：`getColorStateList()` → `getColor()` → `getDrawable()`，**必须 ColorStateList 优先**否则丢日夜切换
- `common_controls_theme.json` 的 Card variant fallback 装饰保持可见（`#FFFFFF + 16px/0px + filter:none`），AAR 缺失时 StyleHelper 仍画出白底圆角卡片

**改 Card 渲染前再读一次源码**：扩展 / 适配公司控件时不能默认"Card 后面是 CardView"，已经不是了。

#### AAR 路径必须用 rootProject.file()

`app/build.gradle` 里的 `file()` 相对 app 模块目录解析，不是项目根。AAR 放在 `demo/sdk/` 而不在 `app/sdk/`，必须用 `rootProject.file('sdk/gua-car-ui-lib-release.aar')` 或 `file('../sdk/...')`。

#### 扩展更多公司控件前的必检维度

1. 是否是 ViewGroup（A2UI 容器组件需要承载 child）
2. 是否破坏 Row/Column 的 Flexbox 语义
3. padding/background/radius/elevation API 是否和 A2UI 原生组件语义一致
4. action/click/function call 是否仍能触发
5. AAR 不存在时 fallback 是否可用
6. 是否引入新 minSdk、manifest 冲突、权限、资源冲突
7. 是否需要真机验证（assembleDebug 通过 ≠ 渲染正确）
