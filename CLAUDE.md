...
Demo 通过 `CommonControlsRegistry` 在 A2UI 渲染层适配公司 Melo/Gua 控件：
...
- **反射加载**：所有 Melo 控件通过 `Class.forName("com.meloui.car.ui.widget.MeloXxx")` 反射创建，无编译期依赖。反射失败自动 fallback 到标准 Android View
- **A2UI 协议无感知**：A2UI JSON 仍使用标准组件名（Text/Card/Row/Column/Button），CardData 协议不变，AI/Card HTTP 服务不返回 Melo/Gua 控件名
- **已适配组件**：Text → MeloTextView、Card → MeloCardView/MeloFrameLayout、Button → MeloFrameLayout 容器（非 MeloButton，因 Button 非 ViewGroup 无法容纳 child）。Row/Column 保留原生 FlexContainerLayout——用 LinearLayout 替代会丢失 stretch、gap、flex-wrap 等 Flexbox 语义
- **minSdk 28**：因公司 AAR 要求，Demo app 的 minSdk 从 21 提升到 28
- **验证方法**：查看 App 日志 `"Common controls available: true/false"` 和 `"Registered common control component: ..."`
