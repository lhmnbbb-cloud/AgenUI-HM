...
| Card 内边距丢失 | 启用公司 CardView 后卡片内容贴边 | `setPadding()` 不等于 CardView 的 `setContentPadding()`，公司 CardView 未 override setPadding | 检查 `MeloCardComponent.applyCardContentPadding()` 是否生效；真机确认内容不贴边 |
...
- **已适配组件**：Text → MeloTextView、Card → MeloCardView、Button → MeloFrameLayout 容器。Row/Column 保留原生 FlexContainerLayout（LinearLayout 替代会丢失 Flexbox 语义）
- **minSdk 28**：因公司 AAR 要求
- **验证方法**：App 日志 `"Common controls available: true/false"`

#### Card padding 是最重要的坑

`StyleHelper.applySpacing()` 对所有 View 调用 `view.setPadding()`，但 CardView 的内容区 padding 语义是 `setContentPadding()`。原生 CardComponent 通过匿名子类 override `setPadding()` → `setContentPadding()` 解决；MeloCardView 没有这个 override。MeloCardComponent 通过 `applyCardContentPadding()` 在 super 调用之后重新解析 styles 并调用 `setContentPadding()` 修复。**适配公司控件不能只看"能创建 View、能编译、能显示"，必须确认 padding/background/radius/click/elevation/Flexbox 等每个 A2UI 组件语义维度一致。**
